"""Pretty-print document metadata statistics reports using rich."""
...
def _format_count(count: int, total: int) -> str:
    ...
    Args:
        count: The sub-count to display (e.g. number of documents
            missing a given key).
        total: The total document count, used as the percentage
            denominator.
...
def _build_overview_line(stats: dict[str, Any], count: int) -> Text:
    """Build the "N docs · N distinct keys" summary line shown at the
    top of the report.
    ...
    Examples:
        ...
        >>> text.plain
        '100 docs  ·  2 distinct keys'
    """
    overview = Text()
    overview.append(f"{count:,} docs", style="bold")
    overview.append("  ·  ")
...
def _build_summary_table(stats: dict[str, Any], count: int) -> Table:
    ...
    Renders a "keys per doc" section (average/min/max) and a "data
    ...
    Args:
        ...
        count: Total document count, used as the percentage
...
def _build_key_flags(info: dict[str, Any]) -> str:
    ...
    Args:
        info: The per-key stats dict (one value of ``stats["per_key"]``),
            expected to contain ``missing_in_docs``, ``value_types``,
    ...
    Examples:
        >>> _build_key_flags({
        ...     "missing_in_docs": 0, "value_types": ["str"],
        ...     "none_or_empty_count": 0,
        ... })
        '[green]ok[/green]'
    """
    flags = []
    if info["missing_in_docs"] > 0:
...
def _build_per_key_table(per_key: dict[str, Any], count: int) -> Table:
    ...
    for key, info in per_key.items():
        table.add_row(
            key,
            _format_count(info["present_in_docs"], count),
