...
##################################################
#     Tests for compute_content_stats_approx     #
##################################################
...
def test_compute_content_stats_approx_returns_dict(docs: list[Document]) -> None:
    assert isinstance(compute_content_stats_approx(docs), dict)


def test_compute_content_stats_approx_does_not_mutate_input(docs: list[Document]) -> None:
    original_len = len(docs)
    compute_content_stats_approx(docs)
...
def test_compute_content_stats_approx_empty_list() -> None:
    assert compute_content_stats_approx([]) == {
        "count": 0,
        "total_chars": 0,
        "avg_chars": 0,
        "std_dev_chars": 0,
...
def test_compute_content_stats_approx_empty_generator() -> None:
    def gen() -> object:
        return
        yield  # pragma: no cover

    assert compute_content_stats_approx(gen()) == {
        "count": 0,
        "total_chars": 0,
        "avg_chars": 0,
        "std_dev_chars": 0,
...
# --- Core analysis (exact fields: count/total/avg/min/max/doc-ids) ---


def test_compute_content_stats_approx_core_exact_fields(docs: list[Document]) -> None:
    result = compute_content_stats_approx(docs, reservoir_size=FULL_RESERVOIR)
...
def test_compute_content_stats_approx_full_reservoir_matches_exact_stats(
    docs: list[Document],
) -> None:
...
def test_compute_content_stats_approx_single_doc() -> None:
...
def test_compute_content_stats_approx_std_dev_known_value() -> None:
...
def test_compute_content_stats_approx_percentiles_one_to_ten() -> None:
    docs = [
...
def test_compute_content_stats_approx_min_max_id_first_on_tie() -> None:
    docs = [
...
def test_compute_content_stats_approx_messy_docs(messy_docs: list[Document]) -> None:
    result = compute_content_stats_approx(messy_docs, reservoir_size=FULL_RESERVOIR)
...
def test_compute_content_stats_approx_none_content_treated_as_empty() -> None:
    doc = Document(id="1", page_content="placeholder", metadata={"source": "x"})
    doc.page_content = None
    result = compute_content_stats_approx([doc], reservoir_size=FULL_RESERVOIR)
    assert result["none_or_non_str_content_count"] == 1
    assert result["empty_count"] == 1
    assert result["min_chars"] == 0


def test_compute_content_stats_approx_non_string_content() -> None:
    doc = Document(id="1", page_content="placeholder", metadata={"source": "x"})
...
def test_compute_content_stats_approx_duplicates(duplicate_docs: list[Document]) -> None:
    result = compute_content_stats_approx(duplicate_docs, reservoir_size=FULL_RESERVOIR)
    assert result["approx_duplicate_count"] == 1


def test_compute_content_stats_approx_no_duplicates() -> None:
    docs = [
    ...
    result = compute_content_stats_approx(docs)
...
def test_compute_content_stats_approx_empty_strings_are_duplicates_of_each_other() -> None:
    docs = [Document(id=str(i), page_content="", metadata={"source": "x"}) for i in range(3)]
    result = compute_content_stats_approx(docs, reservoir_size=FULL_RESERVOIR)
    assert result["approx_duplicate_count"] == 2


def test_compute_content_stats_approx_many_duplicates_same_content() -> None:
    docs = [Document(id=str(i), page_content="repeat", metadata={"source": "x"}) for i in range(5)]
    result = compute_content_stats_approx(docs, reservoir_size=FULL_RESERVOIR)
    assert result["approx_duplicate_count"] == 4


def test_compute_content_stats_approx_duplicate_count_never_undercounts() -> None:
    ...
    result = compute_content_stats_approx(
...
def test_compute_content_stats_approx_generator_input() -> None:
    def gen() -> object:
        yield Document(id="1", page_content="aaaa", metadata={"source": "x"})
        yield Document(id="2", page_content="bb", metadata={"source": "x"})

    result = compute_content_stats_approx(gen(), reservoir_size=FULL_RESERVOIR)
...
def test_compute_content_stats_approx_generator_consumed_only_once() -> None:
    def gen() -> object:
        yield Document(id="1", page_content="x", metadata={"source": "x"})

    g = gen()
    compute_content_stats_approx(g)
    assert list(g) == []


def test_compute_content_stats_approx_map_iterator() -> None:
    raw_texts = ["one", "two", "three"]
    docs_iter = (Document(id=t, page_content=t, metadata={"source": "x"}) for t in raw_texts)
    result = compute_content_stats_approx(docs_iter, reservoir_size=FULL_RESERVOIR)
    assert result["count"] == 3
...
def test_compute_content_stats_approx_reports_approx_flags(docs: list[Document]) -> None:
    result = compute_content_stats_approx(docs)
    assert result["duplicate_count_exact"] is False
    assert result["percentiles_exact"] is False


def test_compute_content_stats_approx_reports_fp_rate(docs: list[Document]) -> None:
    result = compute_content_stats_approx(docs, fp_rate=0.05)
    assert result["bloom_filter_fp_rate"] == 0.05


def test_compute_content_stats_approx_reservoir_capped_below_count() -> None:
    docs = [Document(id=str(i), page_content="x" * i, metadata={"source": "x"}) for i in range(100)]
    result = compute_content_stats_approx(docs, reservoir_size=10)
    assert result["count"] == 100  # exact, unaffected by reservoir size
    assert result["reservoir_sample_size"] == 10  # capped


def test_compute_content_stats_approx_reservoir_uncapped_above_count() -> None:
...
def test_compute_content_stats_approx_percentiles_none_are_estimates_not_exact(
    docs: list[Document],
) -> None:
    result = compute_content_stats_approx(docs)
...
def test_compute_content_stats_approx_thousand_docs() -> None:
    docs = [
        Document(id=str(i), page_content="x" * (i % 50 + 1), metadata={"source": "x"})
        for i in range(1000)
    ]
    result = compute_content_stats_approx(docs, expected_doc_count=1000)
...
########################################
#     Tests for ApproxContentStats     #
########################################


def test_approx_content_stats_starts_at_zero() -> None:
    assert ApproxContentStats().to_dict() == compute_content_stats_approx([])


def test_approx_content_stats_manual_update_calls() -> None:
    stats = ApproxContentStats(reservoir_size=FULL_RESERVOIR)
...
def test_approx_content_stats_to_dict_is_idempotent() -> None:
    stats = ApproxContentStats()
    stats.update(Document(id="1", page_content="abc", metadata={"source": "x"}))
    assert stats.to_dict() == stats.to_dict()


def test_approx_content_stats_accepts_prebuilt_bloom_filter() -> None:
    bloom = BloomFilter(expected_items=10, fp_rate=0.001)
    stats = ApproxContentStats(_bloom=bloom, reservoir_size=FULL_RESERVOIR)
    stats.update(Document(id="1", page_content="abc", metadata={"source": "x"}))
